...
Finalmente inicializamos los motores de físicas individuales para cada uno de los caracteres (enemigos y jugador), además de configurar las cámaras aplicando un bloqueo remporal para suavizar esta transición entre salas e inicializamos la música que hemos elegido para jugar.
Además hacemos uso del método auxiliar ``__door_rect`` para conocer las posiciones de las puertas para poder cruzar de habitación.

#### Métodos de dibujo (`on_draw` y derivados)
El renderizado de los gráficos se centraliza básicamente en la función `on_draw`, desde la que llamamos a funciones que nos ayuda a mostrar la vida del personaje así como el inventario de los objetos que este posee a travñes de la función `draw_info` y la vida de los enemigos presentes en dicha habitación con ``__draw_enemy_hp``. Además mostraremos un mensaje con los enemigos restantes para poder pasar al siguiente nivel.
Luego los enemigos se dibujan a partir de esta  función gracias a la llamada ``self.scene.draw(filter=GL_NEAREST)`` ya que se encuentran todos estos elementos metidos en la escena. Cabe recalcar que durante todo el proyecto hemos hecho uso del filtro __GL_NEAREST__ para mantener la estética retro del juego.
Finalmente también renderizaremos las bombas y los proyectiles cuando sea necesario.
...
- Si la salud del enemigo es menor o igual que cero, se genera una recompensa (`CorazonVida`) en sus coordenadas que cuando se encuentre el personaje con él le añadira vida.
...
Debe de comprobarse que la vida del jugador es mayor que 0, en caso contrario lanzaremos la vista de __GameOverView__ que explicamos anteriormente. Por otro lado, a la hora de que eliminamos todos los enemigos de un nivel comprobamos también si el resto de niveles han sido completados y, por tanto, hemos superado la partida.  

#### Guardar partida
Este es el núcleo de la gestión de partidas guardadas, reinicios, nuevas partidas ...
En ella simplemente creamos un archivo JSON con el nombre de la partida que hemos creado anteriormente, luego itroducimos los datos de los enemígos en ese instante, la dificultad, el estado de las salas y, por supuesto , el estado al completo en el qeu se encuentra nuestro protagonista, así como los objetos que tiene a su disposición.

#### Input de usuario
Registraremos la actividad del usuario para movernos y cambiar de arma a través de los controlese que han sido mostrados por la función ``_draw_controles`` y es plasmada por pantalla al inicio de cada nueva partida.
Resumidamente a la hora de movermos comprobaremos cual es la tecla que ha sido pulsada, la cual nos ayudará a la hora de usar las armas.
Por otro lado, cuando pulsemos __ESCAPE__ abriremos la pantalla de pausa que fue explicada anteriormente.
Por último, __ESPACIO__ es el botón que usamos para usar las armas, por lo que comprobaremos que tenemos seleccionada un objeto, en caso de que no se ignora la pulsación, y en es caso comprobamos si es una bomba o un proyectil, boomerang, y serán añadidos a `lista_bombas` y `lista_proyectiles` respectivamente. Si es la espada con lo que estamos atacando se usará y se añadirá en escena. 
## Elementos del juego
...
A través de las habitaciones haremos que spawneen los enemigos en estas, dependiendo del nivel de dificultad que se selecciono. Dependiendo del nivel de dificultad el número de enemigos, no bosses, será mayor o menor. Por ello se comprueba los diferentes casos y se spawnean dentro de la sala, tratando de mantener una aleatoriedad en cada una de las partida, y comprobando que no spawnee fuera o dentro de un obstáculo lo que generaría muchos bugs. Además, hemos implementado que  estos no puedan aparecer cerca del jugador, sería muy complicado, a no ser que hayamos elegido una partida "Difícil".
...
Los enemigos ordinarios (Enemigo 1, 2 y 3), los cuales poseen algoritmos de persecución básicos que calculan la distancia respecto al `center_x` y `center_y` del jugador para desplazarse hacia él.
  

Los bosses (Boss 1, 2 y 3), los cuales son un tipo de enemigo que posee más puntos de vida y un mayor ataque, suponiendo un mayor desafío al jugador. A su vez, los atributos de estos (cooldown, rango, daño, vida, etc.) dependerán de la dificultad que fue seleccionada.
...
Por otro lado, me ha costado también probar que los botones de las vistas se encontrasen bien posicionados y funcionasen todos los flujos de manera correcta.
